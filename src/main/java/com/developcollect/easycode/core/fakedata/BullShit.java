...
public class BullShit {
    ...
    public static String generator(String title, int length) {
        ...
        //解析字符串
        JSONObject jsonData = JSON.parseObject(JSON_STR);
        ...
        while (content.length() < length) {
            ...
            if (rand < 10) {
                ...
                content.append(((String) famous.get(random.nextInt(famous.size())))
                        .replace("a", (String) before.get(random.nextInt(before.size())))
                        .replace("b", (String) after.get(random.nextInt(after.size()))));
            } else {
                content.append((String) bosh.get(random.nextInt(bosh.size())));
                content = new StringBuilder(content.toString().replace("x", title));
            }
        }

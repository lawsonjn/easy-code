...
@RequestMapping("<#if package.ModuleName??>/cmsmanager/${package.ModuleName}</#if>/<#if controllerMappingHyphenStyle??>${controllerMappingHyphen}<#else>${table.entityPath}</#if>")
<#if kotlin>
...
public class Admin${table.controllerName} {
    ...
    <#if !lombok>
    private static final Logger log = LoggerFactory.getLogger(Admin${table.controllerName}.class);
    </#if>

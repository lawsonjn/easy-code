...
@RequestMapping("<#if package.ModuleName??>/front/${package.ModuleName}</#if>/<#if controllerMappingHyphenStyle??>${controllerMappingHyphen}<#else>${table.entityPath}</#if>")
<#if kotlin>
class Front${table.controllerName}<#if superControllerClass??> : ${superControllerClass}()</#if>
<#else>
<#if superControllerClass??>
public class Front${table.controllerName} extends ${superControllerClass} {
<#else>
public class Front${table.controllerName} {
</#if>
    <#if !lombok>
    private static final Logger log = LoggerFactory.getLogger(Front${table.controllerName}.class);
    </#if>
    ...
    @PostMapping("/delete")
    public JcResult delete(@RequestBody IDsDTO ids) {
        ${cfg.serviceVar}.removeByIds(ids.getIds());
        return JcResult.ok();
    }
    ...
    @PostMapping("/update")
    public JcResult update(@RequestBody ${entity} ${cfg.entityVar}) {
        ${cfg.serviceVar}.update(${cfg.entityVar});
        return JcResult.ok();
    }
